...
def detect_mixed_language(answer, primary_lang='ja',
                          jp_chunk_min_len=10, en_chunk_min_len=30,
                          jp_ratio_threshold=0.3,
                          ja_chunk_threshold=0.5,
                          extra_brand_whitelist=None):
    """
    回答内に「複数言語が混在している」事象を検出する（候補スクリーニング用）。

    判定ロジック:
    - 回答を段落/文単位に分割
    - **同一回答内に「実質的な日本語段落」と「実質的な非日本語段落」が両方存在する** → 混在
    - 単一言語のみで構成された回答は混在ではない（UI設定言語に依存しない）
    - 店舗名・ブランド名・固定名詞は事前に除外（誤検出防止）

    判定例:
    - 冒頭日本語+本文英語(4/11 BUG)        → ✅ 検出
    - 全段落英語(質問が日本語でも回答が英語) → ❌ 検出しない（言語ミスマッチは別事象）
    - 全段落日本語(店舗名が英語でも本文が日本語) → ❌ 検出しない
    - 句読点・中点(・)のみ → 日本語段落として扱わない

    ※ ルールベースなので過剰検出の可能性あり。LLM 二次判定（verify_mixed_language_llm）と
       併用することで「真の混在」のみに絞り込むことを推奨。

    Returns:
        (is_mixed: bool, japanese_chunks: list[str], non_japanese_chunks: list[str])
    """
    ...
    for chunk in chunks:
        ...
        # 「実質的な日本語段落」: ja_chunk_threshold 以上 + 仮名/漢字を最低3文字含む
        # (「・」だけの羅列を日本語段落として誤認しないため)
        kana_kanji_real = len(re.findall(r'[ぁ-ゟァ-ヺ一-鿿]', chunk))  # 中点除外（仮名/漢字のみ）
        if (ratio >= ja_chunk_threshold and
                kana_kanji_real >= 3 and
                len(chunk) >= jp_chunk_min_len):
            japanese_chunks.append(chunk)
            continue

        # 「実質的な非日本語段落」
        if ratio < jp_ratio_threshold and len(chunk) >= en_chunk_min_len:
            ...
            if stopword_count >= 2 and len(words) >= 5:
                non_japanese_chunks.append(chunk)

    is_mixed = len(japanese_chunks) > 0 and len(non_japanese_chunks) > 0
    return (is_mixed, japanese_chunks if is_mixed else [], non_japanese_chunks if is_mixed else [])
...
def verify_mixed_language_llm(questions, answers, api_key, batch_size=20):
    """
    ルールベースで検出された「言語混在」候補を LLM で二次判定する。
    Returns: list of bool（True = LLM が真の混在と確認）
    """
    try:
        import anthropic
    except ImportError:
        print("⚠️  anthropic パッケージが見つかりません。LLM 二次判定をスキップ")
        return [True] * len(answers)  # フォールバック: ルールベース結果を維持
    ...
    print(f"\n🤖 言語混在 LLM 二次判定: {total}件を{batch_size}件ずつ処理中...")
    ...
    for i in range(0, total, batch_size):
        ...
        items_text = ""
        for j in range(n):
            q = str(batch_q[j])[:80] if not pd.isna(batch_q[j]) else ""
            a = str(batch_a[j])[:1500] if not pd.isna(batch_a[j]) else ""
            items_text += f"\n[{j+1}] Q: {q}\n    A: {a}\n"

        prompt = f"""あなたはチャットボット品質監視担当です。
以下の回答について、それぞれ「言語混在」かどうかを判定してください。

【「言語混在」の定義】
ひとつの回答内に、日本語の文章と他言語(英語等)の文章が段落・文レベルで併存している事象。

【混在ではないケース(false にする)】
- 全文が日本語、または全文が英語(質問の言語に関わらず単一言語で完結している)
- 店舗名・ブランド名・固有名詞のみ英語(例: 「NEWoMan高輪のLE PHILをご案内します」)
- 短い専門用語のみ英語(例: Wi-Fi, ATM, FAQ, AI 等)
- 商品名・サービス名のみ英語
- 数値・時刻・URL・Markdown 記号

【混在のケース(true にする)】
- 日本語の文章と英語の文章が両方含まれている(例: 冒頭日本語+本文英語)
- 中国語/韓国語等と日本語が文章レベルで混じっている

評価対象:{items_text}

以下の JSON のみで回答してください(説明不要):
{{"results": [{{"id": 1, "mixed": true, "reason": "..."}}, ...]}}
...
reason は10文字以内で簡潔に。"""
        ...
        try:
            ...
            if json_match:
                ...
                batch_results = data.get("results", [])
                # id 順にソートして取り出す
                by_id = {item.get("id"): item for item in batch_results}
                for j in range(n):
                    item = by_id.get(j + 1, {})
                    results.append(bool(item.get("mixed", True)))
            else:
                raise ValueError("JSONが見つかりません")

        except Exception as e:
            print(f"  ⚠️  バッチ {i//batch_size + 1} でエラー: {e}。ルールベース結果を維持")
            results.extend([True] * n)
...
def fetch_from_api(base_url, token, dataset_id, start_date, end_date, ai_id=None, include_sources=None):
    ...
    4. session.source でフィルタ（デフォルト: widget, share のみ。api/playground/realtime を除外）

    Args:
        include_sources: list[str] | None  含めるソース (例: ['widget', 'share'])。
                         None の場合は全ソースを含める。
    """
    if requests is None:
        print("❌ requests ライブラリが必要です: pip install requests")
    ...
    # Step 3: source でフィルタ
    # session.source の取りうる値: widget / api / playground / share / realtime
    if include_sources is not None:
        included = set(s.strip().lower() for s in include_sources)
        before = len(all_messages)
        excluded_by_source = {}
        filtered = []
        for msg in all_messages:
            session = msg.get("session") or {}
            src = (session.get("source") or "unknown").lower()
            if src in included:
                filtered.append(msg)
            else:
                excluded_by_source[src] = excluded_by_source.get(src, 0) + 1
        all_messages = filtered
        excluded_total = before - len(all_messages)
        if excluded_total > 0:
            breakdown = ", ".join(f"{k}={v}" for k, v in sorted(excluded_by_source.items()))
            print(f"🚫 source フィルタ: {excluded_total} 件除外 ({breakdown})")
            print(f"   許可ソース: {', '.join(sorted(included))}")
            print(f"   フィルタ後: {len(all_messages)} 件")
        if not all_messages:
            print("❌ source フィルタ後のメッセージが 0 件です")
            sys.exit(1)

    # Step 4: API レスポンス → CSV互換 DataFrame に変換
    rows = []
    for msg in all_messages:
        ...
        # session.source（widget/api/playground/share/realtime）
        session_obj = msg.get("session") or {}
        source = (session_obj.get("source") or "unknown").lower()
...
def main():
    ...
    # Source filter (API mode)
    parser.add_argument(
        '--include-sources',
        default='widget,share',
        help='Comma-separated session sources to include (default: widget,share). '
             'Options: widget,api,playground,share,realtime. Pass "all" to disable filtering.',
    )
    ...
    if args.csv:
        ...
        # Parse --include-sources
        if args.include_sources.strip().lower() == 'all':
            include_sources = None
        else:
            include_sources = [s.strip() for s in args.include_sources.split(',') if s.strip()]

        df = fetch_from_api(
            args.api_url, args.token, args.dataset_id,
    ...
    # 言語混在検出（2段階判定）
    # ステップ1: ルールベースで候補を抽出
    # ステップ2: LLM で語義的に「真の混在」か再確認（--use-llm 時のみ）
    ...
    df['混在日本語段落'] = mixing_results.apply(lambda r: r[1])
    df['混在異常段落'] = mixing_results.apply(lambda r: r[2])

    rule_based_count = int(df['言語混在'].sum())
    print(f"\n📌 言語混在検出（ルールベース候補）: {rule_based_count}件")

    # LLM 二次判定: ルールベース候補に対して語義的に再確認
    if args.use_llm and rule_based_count > 0:
        anthropic_key = args.anthropic_key or __import__('os').environ.get('ANTHROPIC_API_KEY')
        if anthropic_key:
            candidates = df[df['言語混在']]
            confirmed_flags = verify_mixed_language_llm(
                candidates['質問'].tolist(),
                candidates['回答'].tolist(),
                anthropic_key,
            )
            # LLM が False と判定したものを言語混在から除外
            for idx, ok in zip(candidates.index, confirmed_flags):
                if not ok:
                    df.at[idx, '言語混在'] = False
                    df.at[idx, '混在日本語段落'] = []
                    df.at[idx, '混在異常段落'] = []
            print(f"   LLM 確認後: {int(df['言語混在'].sum())}件（{rule_based_count - int(df['言語混在'].sum())}件は誤検出として除外）")
        else:
            print("   ⚠️  ANTHROPIC_API_KEY 未設定。LLM 二次判定をスキップ（ルールベース結果を採用）")
    ...
    print(f"📌 言語混在最終: {mixed_language_count}件 ({mixed_language_rate:.2f}%)")
    ...
    if mixed_language_count > 0:
        ...
        for _, row in df[df['言語混在']].head(10).iterrows():
            ...
            q = escape_html(str(row.get('質問', ''))[:120])
            full_answer = escape_html(str(row.get('回答', '')))
            sample_rows_html += f'''
                        <tr>
                            <td style="white-space:nowrap;font-size:0.85rem;vertical-align:top;">{ts}</td>
                            <td style="font-size:0.85rem;vertical-align:top;">{q}</td>
                            <td style="font-size:0.85rem;color:var(--text-muted);vertical-align:top;">
                                <div style="max-height:200px;overflow-y:auto;white-space:pre-wrap;word-break:break-word;line-height:1.5;padding:4px 0;">{full_answer}</div>
                            </td>
                        </tr>'''
